...
static void
shm_monitor(SynapticsSHM *synshm)
{
    int header = 0;
    SynapticsSHM old;
    double t0 = get_time();
...
SynapticsSHM* boo(){SynapticsSHM* a; return a;}
...
static void
shm_process_commands(int do_monitor)
{
    SynapticsSHM *synshm = NULL;
...
static void
dp_get_settings(Display *dpy, XDevice *dev,int *xmin, int *xmax, int *ymin, int *ymax, int *zmin, int *zmax)
{
    int j;
    Atom a, type, float_type;
    int format;
    ...
    //printf("Parameter settings:\n");
    for (j = 0; params[j].name; j++) {//j<10; j++) {//
	struct Parameter *par = &params[j];
        //printf("0");
	a = XInternAtom(dpy, par->prop_name, True);
	if (!a) {
...
//printf("3");
   ...
   // printf("made it!");
		...
		i = (long*)data;
                //printf("    %-23s 32= %ld\n", par->name, i[par->prop_offset]);
                //printf("uglug\n");
                if(params[j].name=="LeftEdge"){
                    *xmin = (int)i[par->prop_offset];
                }else if(params[j].name=="RightEdge"){
                    *xmax = (int)i[par->prop_offset];
                }else if(params[j].name=="TopEdge"){
                    *ymin = (int)i[par->prop_offset];
                }else if(params[j].name=="BottomEdge"){
                    *ymax = (int)i[par->prop_offset];
                }else if(params[j].name=="PressureMotionMinZ"){
                    *zmin = (int)i[par->prop_offset];
                }else if(params[j].name=="PressureMotionMaxZ"){
                    *zmax = (int)i[par->prop_offset];
                }
                //printf("uglug\n");
		break;
                //printf("after break\n");
...
int initsynclient(int *xmin, int *xmax, int *ymin, int *ymax, int *zmin, int *zmax)
{
    ...
    dp_get_settings(dpy,dev,xmin,xmax,ymin,ymax,zmin,zmax);
    XCloseDevice(dpy, dev);
    XCloseDisplay(dpy);

    /*return 1 if could not get bounds*/
